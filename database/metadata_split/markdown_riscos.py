...
def process_riscos_markdown(markdown_text, filepath):
    ...
    if 'riscos_structured.md' in filepath:
        unidades = re.split(r'(## Unidade:)', markdown_text)
        if len(unidades) <= 1:
            raise ValueError(
                '❌ O arquivo Markdown não possui unidades no formato esperado (## Unidade:).'
            )
        ...
        raise ValueError("❌ O arquivo fornecido não é 'riscos_structured.md'.")
